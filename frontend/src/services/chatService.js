...
const chatService = {
  ...
  getUserChats: async (limit = 50, offset = 0) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/history/chats?limit=${limit}&offset=${offset}`);
  ...
  getChatById: async (chatId, limit = 50, offset = 0) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/history/chats/${chatId}?limit=${limit}&offset=${offset}`);
      return response.data;
    } catch (error) {
  ...
  sendMessage: async (userMessage, chatId) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/v1/chat/completions`, {

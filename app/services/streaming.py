...
from app.utils.gemini_mapper import create_streaming_gemini_chunk
    ...
    async def streaming_chat(self, request: LLMRequest, current_user: User) -> StreamingResponse:
        try:
            async def generate_stream() -> AsyncGenerator[str, None]:
                first_chunk = await create_streaming_gemini_chunk(role="assistant")
                ...
                if "messages" in response and response["messages"]:
                    ...
                    for i in range(0, len(full_content), chunk_size):
                        ...
                        chunk_data = await create_streaming_gemini_chunk(content=content_chunk)
                        yield f"data: {json.dumps(chunk_data)}\n\n"

                final_chunk = await create_streaming_gemini_chunk(finish_reason="stop")
